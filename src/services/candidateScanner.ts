...
import { AtrCache, VolatilityCandidate } from "../types";
import { logger } from "../utils/logger";

export async function scanVolatilityCandidates(atrCache: AtrCache): Promise<VolatilityCandidate[]> {
  ...
  return lastValueFrom(
    from(symbols).pipe(
      ...
      filter((candidate): candidate is VolatilityCandidate => Boolean(candidate)),
      toArray(),
