...
import { Candle, TradeIntent, VolatilityCandidate } from "../types";
...
function isKlineEvent(msg: any): msg is WsKlineEvent {
  return Boolean(msg && msg.eventType === "kline" && msg.kline && msg.symbol);
}
...
export function watchCandidatesForSignals(
  ...
  const base$ = fromEvent(ws as any, "formattedMessage").pipe(
    filter(isKlineEvent),
    filter((event) => event.kline.interval === "5m"),
    filter((event) => topics.includes(`${event.symbol.toLowerCase()}@kline_5m`)),
  ...
  const streams = candidates.map((candidate) => {
    ...
    return shared$.pipe(
      ...
      pairwise(),
      map(([prev, curr]) => toTradeIntent(candidate.symbol, candidate.atr, prev, curr)),
