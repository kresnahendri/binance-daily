...
import { refreshAtrCache, loadAtrCache } from "./services/atrService";
...
async function runCandidateJob(): Promise<void> {
  try {
    ...
    logger.info({ symbols: candidates.map((c) => c.symbol) }, "Watching candidates");
    ...
    watchCandidatesForSignals(candidates, config.strategy.monitorMinutes).subscribe({
      next: async (intent) => {
        try {
          await executeTrade(intent);
        } catch (error) {
          logger.error({ symbol: intent.symbol, error }, "Failed to execute trade");
          await sendTelegramMessage(
