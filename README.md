...
- Exits: set `TIME_BASED_EXIT_HOURS` (default 20h) for max holding time, `PROFIT_TRIGGER_PCT` (default 5%) and `LOCK_PERCENT_OF_TRIGGER` (default 0.6) to lock in a portion of gains once the trigger is hit (no static TP); monitor interval via `POSITION_CHECK_INTERVAL_SEC`. Stop-loss uses the wider of 1×ATR or a loss equal to `STOP_LOSS_BALANCE_PCT` of balance (default 1%).
